 ...
 * BACKFILL PAGE 6
 * 
 * "How much doubt yesterday?" (1–10)
 * 
 * Collects: rating_doubt (integer 1-10)
 * Label under "10": "Lots of doubt"
 * 
 * Next → page 7
 */
...
export default function BackfillPage6() {
    const [selectedRating, setSelectedRating] = useState<number | null>(null)
    const [loading, setLoading] = useState(false)
    const [error, setError] = useState<string | null>(null)

    useEffect(() => {
        logPageVisit('v1-bf-6')
    }, [])
    ...
    async function handleSubmit() {
        ...
        formData.set('metricKey', 'rating_doubt')
        formData.set('rating', selectedRating.toString())
        formData.set('nextPage', '/morning/backfill/7')
    ...
    return (
        <div className="w-full max-w-md text-center">
            {/* Page counter */}
            <p className="text-gray-600 text-sm mb-16">6 / 9</p>

            {/* Main question */}
            <h1 className="text-2xl font-semibold text-white mb-12 leading-relaxed">
                How much doubt yesterday?
            </h1>
